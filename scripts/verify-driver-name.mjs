...
try {
 ...
 await page.getByLabel('Your name',{exact:true}).fill('Syam');
 ...
 assert.equal(await canvas.getAttribute('data-driver-name'),'');
 await page.keyboard.press('f');
 await page.waitForFunction(()=>document.querySelector('.drive-world canvas')?.dataset.driverName==='Syam');
 await page.screenshot({path:'/tmp/driver-name.png'});
 await page.keyboard.press('f');
 await page.waitForFunction(()=>document.querySelector('.drive-world canvas')?.dataset.driverName==='');
 assert.deepEqual(errors,[]);
 console.log('PASS: entered name shown when driving and hidden after exit; no rendering errors (mock map).');
