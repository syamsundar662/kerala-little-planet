...
export function createPedestrians(globe:T.Group,obstacles:Obstacle[],latitude:(t:number)=>number){
 ...
 const point=(t:number,side:number)=>{const lat=latitude(t)+side*.135;return new T.Vector3(Math.cos(t)*Math.cos(lat),Math.sin(lat),Math.sin(t)*Math.cos(lat))};
 const clear=(n:T.Vector3)=>staticObstacles.every(o=>Math.acos(T.MathUtils.clamp(n.dot(o.normal),-1,1))*5.6>(o.radius??Math.hypot(o.halfX??0,o.halfZ??0))+.07);
 for(let i=0;i<8;i++){
 ...
 const update=(dt:number,busPosition:T.Vector3)=>{for(const person of people){const next=person.t+person.direction*dt*.035,n=point(next,person.side);let moving=clear(n);if(!moving)person.direction*=-1;
  if(busPosition.lengthSq()>1&&Math.acos(T.MathUtils.clamp(n.dot(busPosition.clone().normalize()),-1,1))*5.6<1.05)moving=false;
