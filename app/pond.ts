...
export function createPond(globe:T.Group,terrain:T.BufferGeometry){
 ...
 const anchor=normal.clone().multiplyScalar(5.6),waterLevel=-.025;
 ...
 for(let i=0;i<positions.count;i++){const p=new T.Vector3().fromBufferAttribute(positions,i);if(Math.abs(p.length()-5.6)<.00001){p.normalize();terrainNormals.setXYZ(i,p.x,p.y,p.z)}}
 ...
 const make=(flat:boolean)=>{
  ...
  water.rotation.x=-Math.PI/2;water.position.y=flat?.005:waterLevel;water.material.uniforms.size.value=8;group.add(water);waters.push(water);
  ...
  for(let ring=0;ring<3;ring++)for(let i=0;i<=96;i++){
   ...
   const y=flat?[.012,.024,-.003][ring]:[waterLevel+.008,waterLevel+.027,Math.sqrt(5.6**2-x*x-z*z)-5.6-.003][ring];v.push(x,y,z);uv.push(x*2,z*2);
 ...
 return {update(time:number,light?:T.DirectionalLight){for(const water of waters){water.material.uniforms.time.value=time*.001;if(light){water.material.uniforms.sunColor.value.copy(light.color).multiplyScalar(light.intensity/3.4);water.material.uniforms.sunDirection.value.copy(light.position).normalize()}}},dispose(){normals.dispose();for(const water of waters){water.material.uniforms.mirrorSampler.value?.dispose();water.material.dispose()}}};
}
