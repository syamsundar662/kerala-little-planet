...
export type Bus={id:number;group:T.Group;state:DriveState;bay:{normal:T.Vector3;heading:T.Vector3}};
...
export function createBusFleet(globe:T.Group,pos:(t:number,lat:number,r?:number)=>T.Vector3,obstacles:Obstacle[]){
 ...
 for(let i=0;i<FLEET_SIZE;i++){
  ...
  attach(new T.CylinderGeometry(.055,.065,1.02,10),pillarMat,moveAlong(bn,faceRoad,.98),.51);       // front pillar
  ...
  obstacles.push({normal:moveAlong(bn,faceRoad,-.98),radius:.42});
  obstacles.push({normal:moveAlong(bn,faceRoad,.98),radius:.08});
 }
 ...
 new GLTFLoader().load('/ksrtc.glb',gltf=>{
  ...
  for(const bus of buses){
   ...
   batchStaticMeshes(model,m=>clonedGlass.has(m));
   bus.group.remove(bus.group.children[0]);bus.group.add(model);
  }
 },undefined,()=>{/* keep placeholders on load failure */});

 const place=(bus:Bus)=>{const n=bus.state.normal,rgt=new T.Vector3().crossVectors(bus.state.heading,n).normalize();bus.group.position.copy(n).multiplyScalar(surfaceRadius(n)+BUS_LIFT);bus.group.quaternion.setFromRotationMatrix(new T.Matrix4().makeBasis(bus.state.heading,n,rgt))};
