...
export function addLandscape(globe:T.Group, obstacles:Obstacle[], latitude:(t:number)=>number) {
 ...
 const anchor=(t:number,lat:number)=>{const g=new T.Group();g.position.set(Math.cos(t)*Math.cos(lat),Math.sin(lat),Math.sin(t)*Math.cos(lat)).multiplyScalar(WORLD_RADIUS);g.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),g.position.clone().normalize());globe.add(g);return g};
 ...
 for(let patch=0;patch<280;patch++){
  const t=random()*Math.PI*2,lat=latitude(t)+(random()<.5?-1:1)*(.16+random()*.65);if(!isDryLand(t,lat)||roadOffset(t,lat)<.80)continue;const candidate=new T.Vector3(Math.cos(t)*Math.cos(lat),Math.sin(lat),Math.sin(t)*Math.cos(lat));if(obstacles.some(o=>Math.acos(T.MathUtils.clamp(candidate.dot(o.normal),-1,1))*WORLD_RADIUS<(o.radius??Math.hypot(o.halfX??0,o.halfZ??0))+.25))continue;const g=anchor(t,lat);g.name='Meadow grass';g.userData.groundedVegetation=true;
 ...
 {
  const g=anchor(1.35,1.12);g.name='Connected mountain peaks';
  const verts:number[]=[],cols:number[]=[],ids:number[]=[],uvs:number[]=[],steps=100,width=2.65,depth=1.60;
  for(let z=0;z<=steps;z++)for(let x=0;x<=steps;x++){
   ...
   const base=Math.sqrt(WORLD_RADIUS**2-px*px-pz*pz)-WORLD_RADIUS;
  ...
  obstacles.push({normal:g.position.clone().normalize(),axis:new T.Vector3(1,0,0).applyQuaternion(g.quaternion),halfX:2.50,halfZ:1.45});
 }
 ...
 for(let shrub=0;shrub<46;shrub++){
  ...
  if(!isDryLand(t,lat)||roadOffset(t,lat)<.85||obstacles.some(o=>Math.acos(T.MathUtils.clamp(n.dot(o.normal),-1,1))*WORLD_RADIUS<(o.radius??Math.hypot(o.halfX??0,o.halfZ??0))+.32))continue;
 ...
 for(let i=0;i<720;i++){const t=i/720*Math.PI*2,lat=latitude(t);const p=new T.Vector3(Math.cos(t)*Math.cos(lat),Math.sin(lat),Math.sin(t)*Math.cos(lat)).multiplyScalar((WORLD_RADIUS+.052));const distance=p.distanceToSquared(stop.position);if(distance<closestDistance){closestDistance=distance;roadTarget.copy(p)}}
